...
class AgentConversationControllerIT {
    ...
    @Test
    @DisplayName("given valid user token when chat without conversation id then return chat reply")
    void givenValidUserToken_whenChatWithoutConversationId_thenReturnChatReply() {
        //given
        final RequestBuilder<?, ?> requestBuilder = this.testManager.wiremock()
    ...
    @Test
    @DisplayName("given valid user token when chat with conversation id then return continued chat reply")
    void givenValidUserToken_whenChatWithConversationId_thenReturnContinuedChatReply() {
        //given
        final RequestBuilder<?, ?> requestBuilder = this.testManager.wiremock()
                .createMapping(WireMockEndpoint.POST_CHAT_AGENT)
                .pathPattern(WireMockPathParams.create()
                        .add("agentId", "4e0c95eb-9e63-4b3f-98f4-2c8c713233c0"))
                .applyDefault(context -> context.matchesJson("requestDefaultMappingChatAgentContinueConversation.json")
                        .responseBody("responseDefaultMappingChatAgentContinueConversation.json"))
                .createDefault();
        ...
        this.testManager.mockMvc()
                ...
                .withPathParameters(PathParams.create()
                        .add("agentId", "4e0c95eb-9e63-4b3f-98f4-2c8c713233c0"))
                .applyDefault(context -> context.withRequest("requestDefaultChatAgentContinueConversation.json")
                        .expectResponse("responseDefaultChatAgentContinueConversation.json"))
                .assertDefault();
    ...
    @Test
    @DisplayName("given valid user token when chat with same conversation twice then forward twice")
    void givenValidUserToken_whenChatWithSameConversationTwice_thenForwardTwice() {
        //given
        final RequestBuilder<?, ?> requestBuilder = this.testManager.wiremock()
                .createMapping(WireMockEndpoint.POST_CHAT_AGENT)
                .pathPattern(WireMockPathParams.create()
