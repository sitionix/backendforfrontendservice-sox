...
wait_for_readiness() {
  ...
  while (( attempt <= max_attempts )); do
    if curl -fsS "${SITIONIX_LOCAL_READINESS_URL}" >/dev/null; then
      return 0
    fi
    sleep 2
    ((attempt++))
...
require_env "FORGE_SECURITY_DEV_JWT_SECRET"
...
mkdir -p "${release_backup_root}"
mkdir -p "$(dirname "${SITIONIX_SHARED_SECRET_ENV_PATH}")"
mkdir -p "$(dirname "${SITIONIX_SERVICE_ENV_PATH}")"
...
cat <<EOF > "${SITIONIX_SHARED_SECRET_ENV_PATH}"
FORGE_SECURITY_DEV_JWT_SECRET=${FORGE_SECURITY_DEV_JWT_SECRET}
EOF
chmod 0600 "${SITIONIX_SHARED_SECRET_ENV_PATH}"
...
if ! docker logs "${SITIONIX_CONTAINER_NAME}" --tail 200 | grep -F "profile is active: \"${SITIONIX_ACTIVE_PROFILE}\"" >/dev/null; then
  echo "Container did not log the expected active profile: ${SITIONIX_ACTIVE_PROFILE}" >&2
  docker logs "${SITIONIX_CONTAINER_NAME}" --tail 200 >&2 || true
  rollback_previous_container
  exit 1
fi
